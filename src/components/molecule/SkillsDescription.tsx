...
function SkillsDescription({ skills }: SkillsDescriptionProps) {
  ...
  const description: Record<SkillType["type"], string> = {
    ...
    "spring": "Spring boot은 자바 기반 백엔드 프레임워크로, 보안, 데이터베이스 연동, REST API 구축 등을 체계적으로 구성할 수 있어서 사용했습니다. 특히 엔터프라이즈급 애플리케이션 개발에 강력합니다.",
    ...
    "jenkins": "Jenkins는 지속적 통합 및 배포(CI/CD) 파이프라인을 구성할 때 사용합니다. Git과 연동하여 코드 푸시 시 자동으로 빌드, 테스트, 배포가 이루어지게 하여 프로젝트의 생산성과 안정성을 높이는 데 유용합니다.",
    "socket": "Socket.IO는 실시간 데이터 통신이 필요한 경우 사용합니다. 채팅 기능, 실시간 알림, 라이브 업데이트와 같은 인터랙티브한 기능을 구현할 때 매우 효과적이며, 빠른 반응성과 낮은 지연 시간을 제공합니다.",
    "pinia": "Pinia는 Vue 3에서 공식적으로 권장되는 상태 관리 라이브러리입니다. Vuex보다 문법이 간결하고 타입스크립트와의 호환성이 뛰어나며, 코드의 가독성과 유지보수성을 높일 수 있어 Vue 프로젝트에서 사용합니다.",
    "python": "Python은 데이터 처리, 인공지능, 자동화 작업에 적합한 언어로, 백엔드 API 구현이나 머신러닝 모델 연동 시 주로 사용합니다. 문법이 간단하고 다양한 라이브러리 생태계가 있어 빠르게 프로토타이핑하기에도 좋습니다."
  };
      ...
      <div className="flex flex-wrap gap-4 mb-6">
        {skills.map((item, index) => (
          <div key={`${item}-${index}`} onClick={() => setSelectedSkill(item)} className="cursor-pointer">
            <SkillIcon type={item} color={selectedSkill === item ? "#979797" : "#CDCDCD"} />
          </div>
        ))}
      </div>
